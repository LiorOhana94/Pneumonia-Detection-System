...
dataloaders =  {x : { 'loader': torch.utils.data.DataLoader(dset[x], batch_size=100, num_workers=num_threads, sampler=ImbalancedDatasetSampler(dset[x])), 'length': len(dset[x])}
               for x in categories}
